...
def vae_loss(input_img, output_img):
    ...
    xent = K.sum(
        K.binary_crossentropy(input_img, output_img), axis=1
    )
    # Kullback-Leibler Divergence
    kl = 0.5 * K.sum(
        K.exp(z_var) + K.square(z_mean) - 1.0 - z_var,
        axis=1,
    )
    return xent + kl
...
for i, xi in enumerate(grid_x):
    for j, yj in enumerate(grid_y):
        ...
        print(
            np.min(x_decoded.flatten()),
            np.max(x_decoded.flatten()),
        )
        axs[i][j].imshow(
            x_decoded.reshape(28, 28), cmap="gray"
        )
plt.show()
